...
function updateStatus() {
  const water = inventory.find((entry) => entry.id === "water")?.amount ?? 0;
  const food = inventory.find((entry) => entry.id === "food")?.amount ?? 0;
  const groupSize = Number(groupSizeInput.value) || 0;
  ...
  const waterMissing = water < weatherState.waterConsumption;
  const foodMissing = food < weatherState.foodConsumption;

  if (!waterMissing && !foodMissing) {
    statusBox.classList.add("ok");
    statusText.textContent = "Vorräte ausreichend für 24 Stunden.";
    return;
  }
  ...
  if (waterMissing && foodMissing) {
    statusText.textContent = "Warnung: Wasser und Nahrung auffüllen!";
  } else if (waterMissing) {
    statusText.textContent = "Warnung: Wasservorrat auffüllen!";
  } else {
    statusText.textContent = "Warnung: Nahrungsvorrat auffüllen!";
  }
}
